...
<template>
	<div class="wrapper ">
		<div class="scroll-wrapper">
			...
			<div class="subject-detail-wrapper scroll-wrapper subject-detail-padding pading-bottom">
	...
	export default {
		data() {
			return {
				...
				subjectInfo: '',
				currentTab: 'SubjectInfo', // currentTab 用于标识当前触发的子组件
				currentIndex: 0,
				tabs: [{
					'text': "详情",
					'content': 'SubjectInfo'
		...
		methods: {
			...
			//获取公开课详情
			getSubjectDetail() {
				let data = JSON.parse(sessionStorage.getItem('classItemdata'));
			...
			getSubjectinfo(params) {
				...
				if(localUrl.indexOf('class') > -1){
					v.$api.get('subject/info/' + v.id + '', params, function(r) {
						let subjectinfo = r.data;
						let classTeacher = subjectinfo.userlist;
						sessionStorage.setItem('classTeacher',JSON.stringify(classTeacher));
					})
				}
			},
			//打开机构页面
